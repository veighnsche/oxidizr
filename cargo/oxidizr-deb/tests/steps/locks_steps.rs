...
pub async fn allow_nonroot_sudo_owner(world: &mut World) {
    world.envs.push(("OXIDIZR_DEB_TEST_ALLOW_NONROOT_SUDO_OWNER".into(), "1".into()));
}

#[given(regex = r"^EXDEV degraded fallback is forced via env$")]
pub async fn force_exdev(world: &mut World) {
    world.envs.push(("SWITCHYARD_TEST_ALLOW_ENV_OVERRIDES".into(), "1".into()));
    world.envs.push(("SWITCHYARD_FORCE_EXDEV".into(), "1".into()));
}
