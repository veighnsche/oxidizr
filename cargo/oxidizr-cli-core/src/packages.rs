...
pub const DEST_DIR: &str = "/usr/bin";

pub fn static_fallback_applets(pkg: PackageKind) -> Vec<String> {
    match pkg {
        PackageKind::Coreutils => coreutils_full_list(),
        PackageKind::Findutils => vec!["find".to_string(), "xargs".to_string()],
        PackageKind::Sudo => vec!["sudo".to_string()],
    }
}

pub fn dest_dir_path() -> PathBuf { PathBuf::from(DEST_DIR) }
