...
impl PacmanManager {
    ...
    fn validate_package_name(&self, name: &str) -> Result<()> {
        if name.is_empty() || name.starts_with('-') {
            return Err(CoreutilsError::ExecutionFailed(format!("Invalid package name: {}", name)));
        }
        
        if name.len() > crate::config::security::MAX_PACKAGE_NAME_LENGTH {
            return Err(CoreutilsError::ExecutionFailed("Package name too long".into()));
        }
        
        if !name.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_' || c == '+' || c == '.') {
            return Err(CoreutilsError::ExecutionFailed(format!("Invalid characters in package name: {}", name)));
        }
...
impl PackageManager for PacmanManager {
    ...
    fn install(&self, package: &str) -> Result<()> {
        ...
        if let Some(helper) = self.get_aur_helper() {
            let status = Command::new("sudo")
                .args(["-u", "builder", "--", &helper, "-S", "--noconfirm", "--needed", package])
                .status()?;
        ...
        Err(CoreutilsError::ExecutionFailed(format!("Failed to install '{}'", package)))
    }
    ...
    fn remove(&self, package: &str) -> Result<()> {
        ...
        if status.success() {
            ...
            Err(CoreutilsError::ExecutionFailed(format!("Failed to remove '{}'", package)))
        }
    }
